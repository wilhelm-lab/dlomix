...
class PrositChargeStateAdaption(tf.keras.Model):
    ...
    def __init__(
        ...
        self.embedding = tf.keras.layers.Embedding(
            input_dim=self.embeddings_count, output_dim=embedding_dim, input_length=seq_length)
        ...
        self.regressor = tf.keras.Sequential(
            [
                tf.keras.layers.Dense(
                    self.regressor_layer_size, activation="relu"),
        ...
        self.dense1 = tf.keras.layers.Dense(
            64, activation=tf.keras.layers.LeakyReLU())
