...
class PrositSumIntensityPredictor(tf.keras.Model):
    """Implementation of the Prosit model for intensity prediction.
    ...
    def __init__(
        ...
        super(PrositSumIntensityPredictor, self).__init__()
    ...
    def call(self, inputs, **kwargs):
        log_sum_intensities_in = inputs["sum_intensities"]
        peptides_in = inputs["sequence"]
        collision_energy_in = inputs["collision_energy"]
        precursor_charge_in = inputs["precursor_charge"]

        encoded_meta = self.meta_encoder([log_sum_intensities_in, collision_energy_in, precursor_charge_in])
