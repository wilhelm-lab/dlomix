...
class Report(abc.ABC):
    ...
    def plot_keras_metric(self, metric_name, save_plot=True):
        ...
        if metric_name.lower() not in self._history_dict.keys():
            raise ValueError(
                'Metric name to plot is not available in the history dict. Available metrics to plot are {}', self._history_dict.keys()
                )

        if "val_"+metric_name.lower() not in self._history_dict.keys():
            raise ValueError(
                'No validation epochs were run during training, the metric name to plot is not available in the history dict. Available metrics to plot are {}', self._history_dict.keys()
                )        
