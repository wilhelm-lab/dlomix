...
class RetentionTimeReportWandb:
  ...
  METRICS_TO_EXCLUDE = ['epoch/learning_rate', 'epoch/epoch', 'batch/learning_rate', 'batch/batch_step']

  def __init__(self, project: str, title: str, description: str, dataset: RetentionTimeDataset = None):
    """_summary_
  ...
  def _get_metrics_names(self):
    ...
    strings_to_filter = RetentionTimeReportWandb.METRICS_TO_EXCLUDE
  ...
  def _build_train_section(self):
    batch_train_metrics_names, epoch_train_metrics_names, _ = self._get_metrics_names()
    panel_list_batch = []
    panel_list_epoch = []
