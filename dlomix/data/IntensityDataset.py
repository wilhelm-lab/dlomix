...
from dlomix.utils import convert_nested_list_to_numpy_array, order_intensities, msp_to_pd
...
class IntensityDataset:
    ...
    def __init__(
        self,
        data_source=None, 
        data_file_type='csv', #JL
        ordered=True, #JL
        dic_key_path=None, #JL
        sep=",",
        ...
        mod_inds_col=None,#JL
        mod_names_col=None,#JL
        feature_cols=None,
        normalize_targets=False,
        seq_length=0,
        model_input_specs_path=None, #JL
        batch_size=32,
        val_ratio=0,
        seed=21,
        test=False,
        ...
        self.data_file_type = data_file_type #JL
        self.ordered = ordered #JL
        if not ordered: #JL
            assert dic_key_path is not None #JL
            self.read_dictionary(dic_key_path) #JL
        self.sep = sep
        ...
        #JL
        self.mod_inds_col = mod_inds_col
        self.mod_names_col = mod_names_col
        ...
        #JL
        self.model_input_specs_path = model_input_specs_path
        if model_input_specs_path is not None:
            with open(model_input_specs_path, 'r') as stream:
                self.specs = yaml.safe_load(stream)
            self.seq_length = self.specs['max_seq']
        else:
            self.specs = None
        ...
        self.mod_inds = None #JL
        self.mod_names = None #JL
    ...
    def read_dictionary(self, path):
        self.dic_key = {line.split()[0]: int(line.split()[1]) for line in open(path)}
    ...
    def _read_data(self):
        if isinstance(self.data_source, tuple):
            ...
            if self.data_file_type.lower() == 'csv':
                df = pd.read_csv(self.data_source)
            elif (self.data_file_type.lower() == 'pkl') | (self.data_file_type.lower() == 'pickle'):
                df = pd.read_pickle(self.data_source)
            elif self.data_file_type.lower() == 'msp':
                df = msp_to_pd(self.data_source) # must use df for build_tf_dataset
                self.df = df
            else:
            ...
            #JL
            if self.mod_inds_col is not None:
                self.mod_inds = df[self.mod_inds_col]
            if self.mod_names_col is not None:
                self.mod_names = df[self.mod_names_col]
            #JL
            #if self.ordered:
            ...
            #JL
            # I couldn't figure out how to make my order_intensities() work with dataframe.values
            if self.ordered:
                self.intensities = convert_nested_list_to_numpy_array(
                    self.intensities.values
                )
            else:
                self.intensities = order_intensities(
                    df[self.intensities_col], df['anns'], df['seq'], self.dic_key
                )
    ...
    def _build_tf_dataset(self):
        from dlomix.custom_datasets.predire_dataset import create_dataset #JL
        ...
        for split in self.tf_dataset.keys():
            ...
            #JL
                ...
                inds = self.indicies_dict[split]
                self.tf_dataset[split] = create_dataset(
                    self.df, self.model_input_specs_path, inds, self.batch_size
                )
