...
class PrecursorChargeStateDataset:
    def __init__(
        ...
        if isinstance(classification_type, str):
            ...
            raise TypeError(
                f"Error: {classification_type}. classification_type must be a string."
            )
        ...
        if isinstance(charge_states, list):
            if not all(isinstance(item, int) for item in charge_states):
                raise ValueError(
                    f"Error: {charge_states}. Charge_states must be a list of integers."
                )
        else:
        ...
        if isinstance(dir_path, str):
            if not os.path.isdir(dir_path):
                raise ValueError(
                    f"Error: {dir_path}. dir_path must be a valid directory."
                )
        else:
        ...
        self.training_validation_split = StratifiedShuffleSplit(
            n_splits=1, test_size=self.validation_ratio
        )

        # create training and validation dataset from remaining training + validation (trainval)  split
        self.df_train, self.df_val = create_training_validation_split(
            self.training_validation_df,
...
def keep_desired_charges(df, charge_list=None, min_count=None):
    ...
    if min_count is not None:
        ...
        if len(charge_list) < len(charge_list_prev):
            total_list = [
                charge for charge in charge_list_prev if charge not in charge_list
            ]
            print(
                f"Caution! Removed charge states with less than {min_count} occurrences in the dataset. "
...
def remove_rare_sequence_lengths(df, representation_threshold=100):
    ...
    filtered_lengths = len_counts[len_counts["count"] >= representation_threshold][
        "seq_len"
    ]
...
def encode_charge_states(df, charge_states=None):
    ...
    print(
        f"Step 9/12 complete. Encoded all occuring charge states per unique sequence in a binary vector."
    )
    return df
...
def skip_charges_for_occurrences(df, cutoff=1000):
    ...
    for index, i in enumerate(df["charge_state_vector"].value_counts()):
        ...
        list_v.append(i)
        if (
            has_skipped_charges(df["charge_state_vector"].value_counts().index[index])
            and list_v[index] < cutoff
        ):
...
def create_training_validation_split(df, sssplit, data_type=None):
    ...
    train_indicies_embed, val_indicies_embed = next(
        sssplit.split(trainval_ds_embed, trainval_labels_embed)
    )
    # Distribution
    train_ds_embed, train_labels_embed = (
        trainval_ds_embed[train_indicies_embed],
        trainval_labels_embed[train_indicies_embed],
    )
    val_ds_embed, val_labels_embed = (
        trainval_ds_embed[val_indicies_embed],
        trainval_labels_embed[val_indicies_embed],
    )
    # create two dataframes for training and validation
    if data_type == "multi_label":
        df_train = pd.DataFrame(
            {
                "modified_sequence_vector": train_ds_embed,
                "charge_state_vector": train_labels_embed,
            }
        )
        df_val = pd.DataFrame(
            {
                "modified_sequence_vector": val_ds_embed,
                "charge_state_vector": val_labels_embed,
            }
        )
    else:
        df_train = pd.DataFrame(
            {
                "modified_sequence_vector": train_ds_embed,
                "most_abundant_charge_vector": train_labels_embed,
            }
        )
        df_val = pd.DataFrame(
            {
                "modified_sequence_vector": val_ds_embed,
                "most_abundant_charge_vector": val_labels_embed,
            }
        )
