...
from dlomix.reports.Report_Naim import PDFFile, Report
...
from dlomix.detectability_model_constants import CLASSES_LABELS
...
class evaluation_results:
    ...
    def evaluate(self):
        ...
        expected_str = np.squeeze([str(x[0]) for x in np.argmax(expected, axis = -1)])
        ...
        for i in range(len(predicted)):
            ...
            diff_prob = np.absolute(probabilities[i, expected_int[i]] - probabilities[i, predicted[i]])
        ...
        if (self.TP + self.TN + self.FP + self.FN) > 0:
            binary_accuracy =  (self.TP + self.TN) / (self.TP + self.TN + self.FP + self.FN)
        else:
        ...
        if ((self.TP + self.FP)*(self.TP + self.FN)*(self.TN + self.FP)*(self.TN + self.FN)) > 0:
            MCC = ((self.TP*self.TN) - (self.FP*self.FN)) / np.sqrt(float((self.TP+self.FP)*(self.TP+self.FN)*(self.TN+self.FP)*(self.TN+self.FN)))
        else:
            MCC = None
        
        conf_matrix = {'TP':self.TP, 'TN':self.TN, 'FP':self.FP, 'FN':self.FN}
                
        self.eval_results = {'predictions': predicted,
        ...
        if self.print_results:
            print(f'Binary Accuracy: {round(self.eval_results["binary_accuracy"], ndigits = 2)}')
...
class detectability_report(Report):
    ...
    def __init__(self, 
        ...
        super(detectability_report, self).__init__(output_path, history, figures_ext)

        self.pdf_file = PDFFile("DLOmix - Detectability Report")
        
        self.predictions = predictions
        self.test_size = self.predictions.shape[0] # Removing the last part of the test data which don't fit the batch size
    ...
    def generate_report(self, **kwargs): 
        ...
        self.pdf_file.output(join(self._output_path, "Detectability_evaluation_report.pdf"), "F")
      
    def results_dict_and_df(self): 

        eval_result = evaluation_results(self.predictions, self.targets, threshold = self.threshold)
        self.results_metrics_dict = eval_result.eval_results
        
        target_labels = {0: 'Non-Flyer', 1: 'Weak Flyer', 2: 'Intermediate Flyer', 3: 'Strong Flyer'}
        binary_labels = {0: 'Non-Flyer', 1: 'Flyer'}
        
        df_data_results = self.input_data_df.copy().reset_index(drop = True)
        df_data_results = pd.concat([df_data_results, pd.DataFrame(self.results_metrics_dict['predictions'], 
                                                                   columns = ['Predictions'])], axis = 1)

        for i, label in enumerate(CLASSES_LABELS):

            df_data_results[label] = np.round_(np.array(self.results_metrics_dict['probabilities'])[:, i], decimals = 3)

        df_data_results['Flyer'] = df_data_results['Weak Flyer'] + df_data_results['Intermediate Flyer'] + df_data_results['Strong Flyer']
        #df_data_results['Flyer'] = round(df_data_results['Flyer'], ndigits = 3)
        df_data_results['Binary Predictions'] = np.where(df_data_results['Predictions'] == 0, 0, 1)
        df_data_results['Binary Classes'] = np.where(df_data_results['Classes'] == 0, 0, 1)
        
        sorted_columns = ['Sequences','Proteins', 'Weak Flyer', 'Intermediate Flyer', 'Strong Flyer', 'Non-Flyer', 'Flyer',
                          'Classes', 'Predictions', 'Binary Classes', 'Binary Predictions']
        ...
        if 'Proteins' in df_final_results.columns and self.rank_by_prot:
            ...
            df_final_results = df_final_results.sort_values(by = ['Proteins', 'Flyer', 'Predictions', 'Strong Flyer', 'Intermediate Flyer', 'Weak Flyer'], 
                                                            ascending = [True, False, False, False, False, False]).reset_index(drop = True)
                                                            
            df_final_results['Rank'] = df_final_results.groupby('Proteins')['Flyer'].rank(ascending = False, method = 'first').astype(int)
#           df_final_results['Rank_2'] = df_final_results.groupby('Proteins')['Flyer'].rank(ascending = False, method = 'dense').astype(int)

        else:
            
            df_final_results = df_final_results.sort_values(by = ['Flyer', 'Predictions', 'Strong Flyer', 'Intermediate Flyer', 'Weak Flyer'], 
                                                            ascending = [False, False, False, False, False]).reset_index(drop = True)            
            
            df_final_results['Rank'] = df_final_results['Flyer'].rank(ascending = False, method = 'first').astype(int)
#            df_final_results['Rank_2'] = df_final_results['Flyer'].rank(ascending = False, method = 'dense').astype(int) 
        ...
        df_final_results['Binary Classes'] = df_final_results['Binary Classes'].map(binary_labels)
        df_final_results['Predictions'] = df_final_results['Predictions'].map(target_labels)
        df_final_results['Binary Predictions'] = df_final_results['Binary Predictions'].map(binary_labels)
    ...
    def plot_roc_curve_binary(self): 
        ...
        fpr, tpr, thresholds = roc_curve(np.array(self.results_report_df['Binary Classes']), np.array(self.results_report_df['Flyer']))
        AUC_score = auc(fpr, tpr)

        #create ROC curve

        plt.plot(fpr,tpr)
        plt.title('Receiver operating characteristic curve (Binary classification)', y= 1.04, fontsize = 10)
        plt.ylabel('True Positive Rate')
        plt.xlabel('False Positive Rate')
        save_path = join(self._output_path, "ROC_curve_binary_classification" + self._figures_ext)
    ...
    def plot_confusion_matrix_binary(self): 
        ...
        """
        conf_matrix = confusion_matrix(self.results_report_df['Binary Classes'], self.results_report_df['Binary Predictions'])

        conf_matrix 
        conf_matrix_disp = ConfusionMatrixDisplay(confusion_matrix = conf_matrix, display_labels = ['Non-Flyer','Flyer'])
        fig, ax = plt.subplots() 
        conf_matrix_disp.plot(xticks_rotation = 45, ax=ax)
        plt.title('Confusion Matrix (Binary Classification)', y= 1.04, fontsize = 11) 
        save_path = join(self._output_path, "confusion_matrix_binary" + self._figures_ext)
    ...
    def plot_roc_curve(self):
        ...
        for i in range(len(CLASSES_LABELS)):
            fpr[i], tpr[i], _ = roc_curve(np.squeeze(self.targets)[:, i], np.squeeze(self.predictions)[:, i])
        ...
        save_path = join(self._output_path, "ROC_curve_multiclass_classification" + self._figures_ext)
        plt.savefig(save_path, bbox_inches = 'tight', dpi = 90)
        plt.show()
        plt.close()
    ...
    def plot_confusion_matrix_multiclass(self):
        ...
        multi_conf_matrix = confusion_matrix(self.results_report_df['Classes'], self.results_report_df['Predictions'])

        conf_matrix_disp = ConfusionMatrixDisplay(confusion_matrix = multi_conf_matrix, 
                                                  display_labels = CLASSES_LABELS) # 
        fig, ax = plt.subplots() 
        conf_matrix_disp.plot(xticks_rotation = 45, ax=ax)
        plt.title('Confusion Matrix (Multiclass Classification)', y= 1.04, fontsize = 11) #, y=1.12
        save_path = join(self._output_path, "confusion_matrix_multiclass" + self._figures_ext)
        plt.savefig(save_path, bbox_inches = 'tight', dpi = 80)
        plt.show()
        plt.close()
    ...
    def plot_heatmap_prediction_prob_error(self): 
        ...
        for k, v in self.results_metrics_dict['delta_prob_pred'].items():
            ...
            for m, n  in self.results_metrics_dict['delta_prob_pred'][k].items():
                ...
                probability_var[k][m] = round(np.mean(self.results_metrics_dict['delta_prob_pred'][k][m]), ndigits = 3)
...
#                 probability_var_with_std[k][m]['std'] = round(np.std(self.results_metrics_dict['delta_prob_pred'][k][m]), ndigits = 3)
        ...
        plt.title('Heatmap of average error between probabilities of real classes vs predicted', y= 1.04, fontsize = 11) # , y=1.12
        save_path = join(self._output_path, "heatmap_prediction_prob_error" + self._figures_ext)
        plt.savefig(save_path, bbox_inches = 'tight', dpi = 80)
        plt.show()
        plt.close()
...
class predictions_report:
    ...
    def __init__(self, predictions, input_data_df, output_path, num_clases = len(CLASSES_LABELS),rank_by_prot = False , threshold = None):
    ...
    def evaluate(self):
        ...
        df_data_results = self.input_data_df.copy().reset_index(drop=True)
        df_data_results = pd.concat([df_data_results, pd.DataFrame(self.all_pred, 
                                                                   columns = ['Predictions'])], axis = 1)

        for i, label in enumerate(CLASSES_LABELS):

            df_data_results[label] = np.round_(np.array(self.predictions)[:, i], decimals = 3)
        ...
        if 'Classes' in df_data_results.columns:
            ...
            df_data_results['Binary Classes'] = df_data_results['Binary Classes'].map(binary_labels)
        ...
        if 'Proteins' in df_data_results.columns and self.rank_by_prot:
            ...
            df_data_results = df_data_results.sort_values(by = ['Proteins', 'Flyer', 'Predictions', 'Strong Flyer', 'Intermediate Flyer', 'Weak Flyer'], 
                                                          ascending = [True, False, False, False, False, False]).reset_index(drop = True)
                                                            
            df_data_results['Rank'] = df_data_results.groupby('Proteins')['Flyer'].rank(ascending = False, method = 'first').astype(int)
#            df_data_results['Rank_2'] = df_data_results.groupby('Proteins')['Flyer'].rank(ascending = False, method = 'dense').astype(int)

        else:
            
            df_data_results = df_data_results.sort_values(by = ['Flyer', 'Predictions', 'Strong Flyer', 'Intermediate Flyer', 'Weak Flyer'], 
                                                          ascending = [False, False, False, False, False]).reset_index(drop = True) 

            df_data_results['Rank'] = df_data_results['Flyer'].rank(ascending = False, method = 'first').astype(int)
#            df_data_results['Rank_2'] = df_data_results['Flyer'].rank(ascending = False, method = 'dense').astype(int)  
        
        df_data_results['Predictions'] = df_data_results['Predictions'].map(target_labels)
        df_data_results['Binary Predictions'] = df_data_results['Binary Predictions'].map(binary_labels)
