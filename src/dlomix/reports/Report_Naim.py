...
class Report(abc.ABC):
    ...
    def plot_keras_metric(self, metric_name, save_plot=True):
        ...
        plt.xlabel("epoch")
        plt.legend(["train", "val"], loc = "best") # Modification Naim original: plt.legend(["train", "val"], loc="upper left")
        if save_plot:
            save_path = join(self._output_path, metric_name + self._figures_ext) 
            plt.savefig(save_path, bbox_inches = 'tight', dpi = 90)  # Modification Original plt.savefig(save_path)
        plt.show()
        plt.close()
...
class PDFFile(FPDF):
    ...
    def _add_section_content(self, section_title, section_body):
        ...
        if section_body != "":
            ...
            self.ln(2 * PDFFile.LINE_HEIGHT) # Modification Naim original self.ln(PDFFile.LINE_HEIGHT) 
    ...
    def add_content_plot_page(self, plot_filepath, section_title="", section_body=""):
        ...
        self._create_first_page_if_document_empty()
        self.add_page() # new line (Modification Naim)
        self._add_section_content(section_title, section_body)
